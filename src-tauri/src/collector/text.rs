//! -R 源文本解码：UTF-8 优先,非法字节段按系统 ANSI 代码页回退（缺陷 A）。
//!
//! 背景：部分 Agent 在 Windows 上把工作目录等路径字段按系统代码页（中文系统 = GBK / CP936）写出,
//! 若按 UTF-8 lossy 解码会落成 U+FFFD 替换符,再经归一化就成了乱码 project_key。
