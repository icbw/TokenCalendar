//! 悬浮球贴边停靠——「拖到屏幕边缘松手自动贴边收起成竖条,从边缘拖出/双击展开回
//! 卡片」的 edge docking 范式。多显示器（含混合 DPI）下形态与位置必须稳定,几何模型
//! 因此立在以下硬约束上：
//!
...
//!
//! 2. **判定对象按语义分档**：
//!     **自由态表盘看光标**——手感就是「把鼠标推到屏边」,光标即鼠标点击/拖动
//!    位置,**参照屏仍是内容所在屏**（光标只是测量点,不参与选屏）,几像素容差只是
//!    「检测屏缘的兼容范围」;用内容矩形判会明显提前（展开态内容矩形右侧还挂着
//!    6px 间隙 + 16px 按钮列,表盘离缘还有二三十像素就会收起）;
//!     **已停靠保持/拖离与钳制看可见内容矩形**——竖条贴缘时本体离缘 4px、光标
//!    停在主体中部,拿光标判会立刻误判离开;窗口矩形也不行（含透明边距/提示位
//!    画布,越界 ≠ 内容越界）;
//!    ′ **自由态竖条也看可见内容矩形**（与同尺同容差）——竖条的内容就是本体,
//!    不存在表盘那种「按钮列提前触发」的顾虑;若按光标判,光标在竖条中部、离缘远大于
//!    容差,屏内拖动又触发不了的跨缝补判,竖条贴近缘边也吸附不上;
//!     **跨缝补判（只服务自由态表盘）**——拖动中窗口中心换过屏（系统已按「窗口
//!    过半」把窗口判给新屏、DPI 也跟着切）时,光标必然已经随拖动深入新屏、离缝远超
//!    eps ⇒ 整条失效（从接缝左侧拖到右侧,表盘会停在缝上既不贴边也不弹开;反向
//!    因为「越过也算」的外侧无界而照常触发）。补判改用**主体是否完整显示**这条
//!    口径：主体离新屏近缘超过几像素 = 已整体进入新屏 ⇒ 不再贴边;仍压着缝（尚未
//!    完整显示）才算贴。见 `seam_landing_target`。
//!    屏归属 = **窗口中心**：两态容器都绕内容（视觉）中心对称 ⇒ 窗口中心 =
//!    视觉中心,与系统按窗口中心的归属判定同尺。
//!
//! 3. **参照屏唯一**。松手只在**内容当前所在屏**（自由态与已停靠态都是这一台,
//!    光标所在屏/历史停靠屏都不做候选）上解一个问题：本路的判定对象离该屏两条竖缘
//!    最近的一条是否触缘。若盯「停靠状态记录的那台屏、那条缘」,竖条从别处拖到另一台
//!    屏的边缘时会因「离原停靠缘很远」被判拖离（弹出表盘）,跨接缝后还会按旧屏归位
//!    （贴边距离漂移、甚至侵入接缝）。
//!    贴到某条缘 ⇒ 竖条（保持/进入,含换屏换边）;未贴且已停靠 ⇒ 展开、未贴且自由
//!    ⇒ 钳回（自由态竖条吸附后即成为已停靠,再拖离才展开）。
//!    判定对象与容差见硬约束 2 与 `EDGE_EPS_LOGICAL` / `DOCK_KEEP_TOLERANCE_LOGICAL`。
//!
//! 4. **一次归位一个执行者**。形态切换（尺寸）与位置补偿必须在同一次调用里原子
//!    完成：前端 set_orb_size（内容锚定）与 Rust 归位各自补偿一次位置的话,两个
//!    invoke 并发时补偿被算两遍,卡片会横向窜两百像素。
//!
//! 5. **画布不抢鼠标：主体之外动态让出**。窗口绕表盘对称展开（580×310),
//!    画布（内容之外的透明区)只承载 hover 提示,不该拦鼠标。静态
//!    `HTTRANSPARENT`（WM_NCHITTEST）只覆盖「光标从窗口外进入画布」的通路——
//!    从主体滑入画布时光标始终没离开本窗口矩形,系统不会重询命中测试,消息继续
//!    归本窗口（画布会遮挡其他程序的 hover 与操作,右键也仍被响应）。
//!    补一条**动态让出**,两道机制一起上：
//!     `set_window_body_region`——**硬保证**：光标离开交互主体即把窗口区域
//!      （`SetWindowRgn`）收紧到「主体 + 阴影余量」,区域外的画布在几何上不属于
//!      窗口,命中测试绝不会命中（只设 `WS_EX_TRANSPARENT` 样式位不够,画布照样遮挡）;
...
//! tao 按「保持逻辑尺寸」重排窗口（尺寸与位置都会被系统改）。而拖动期间排队的那条消息
//! **在 WM_EXITSIZEMOVE 之后**才到 ⇒ 松手瞬间写完的归位会被它整段抹掉。
//! 因此三条一起成立：
//!  **尺寸写物理值**——`set_size（LogicalSize)` 的换算在 tao 内部用**消息驱动**的
//!   scale 缓存,跨屏松手时缓存还是旧屏值（如目标屏 @1.5 的 56×116 逻辑被按旧屏 2.0
//!   写成 112×232 物理,而非 84×174）;物理值不经缓存换算,写的就是目标矩形,
...
//!   写入幂等（`write_rect` 已达成即静默跳过）,重放不会喂养风暴;移动循环进行中不重放
//!   （那段时间用户的手是几何的主人）。
//!
//! 其余机制：
...
/// 写形态并**变化即落盘**：形态是启动恢复的输入（恢复上次退出前的形态与位置）;
/// 低频离散事件（双击/拖动松手/贴边）,与 `set_dock_state` 同一条落盘纪律。
/// 失败静默——下次显隐/退出兜底重写。
...
pub struct OrbDockState {
    /// 停靠缘："left" | "right"（serde rename;不支持顶/底）。
...
/// 尺寸与位置在同一调用里完成（前端不另调 set_orb_size,避免两次位置补偿）。
///
/// `expand_w` / `expand_h`（逻辑像素,可选）：前端期望的展开窗口尺寸,只与 Rust 侧
/// 展开态常量比对、漂移时告警;实际尺寸恒取常量。
///
...
/// 前端不按 window.innerWidth 猜形态：页面可能在 restore 归位之前就已加载,那时窗口
/// 还是配置初始尺寸（56×116 竖条）——猜错后挂载对齐会把刚恢复的表盘缩回竖条。
/// IPC 经启动闸门,此查询必在 restore 之后执行。
#[tauri::command]
...
/// 未贴边一律表盘并在记录位置原地恢复、**首次启动默认展开（表盘）+ 居中**。
...
mod win {
    ...
    /// ⚠ 单独设这个位**不足以**让画布不再遮挡——样式设置成功后画布照旧吃消息、
    /// 挡住下层程序。真兜底的是 `set_window_body_region`（`SetWindowRgn` 窗口区域收紧,
    /// 几何级硬保证）;本位保留作系统级快路径。
    ...
    /// 不可,但旧式非客户区绘制按**样式**画：只摘 CAPTION 时闪出的框架里仍剩
    /// 「最小化 + 关闭」两个按钮（= 残留的 SYSMENU | MINIMIZEBOX）。
    ...
    ///  自由态**光标触缘**：拖动松手时**以光标（鼠标点击/拖动位置）为起点**算到
    ///    工作区左/右缘的距离,≤ 此值（或已越过该缘）才算「拖到边上了」,松手收成竖条;
    ///    它只是**检测屏幕边缘的兼容范围**,不是吸附范围;判据是光标不是内容矩形
    ///    （展开态内容矩形右侧挂着 6px 间隙 + 16px 按钮列,拿内容判会让表盘离缘二三十
    ///    像素就收起）。
    ///  自由态**跨缝补判的主体完整显示**分界：
    ...
    /// （55,55) 处）——容器因此绕视觉中心对称。与前端 CSS 同源;盘径变化时偏移随之
    /// 重算（与前端 EXPANDED_PAD 同步改）。
    ...
    /// 不该因为推得太远反而被判成「离线很远」。
    /// ⚠ 只服务**按内容矩形判定的一路**（已停靠与自由态竖条;自由态表盘看光标,
    /// 见硬约束 2;跨缝补判）。
    ...
    /// 按内容矩形判定的松手落点（已停靠与自由态竖条）→ 贴边目标：在**内容所在屏**的
    /// 两条竖缘里取最近的一条,间距 ≤ tol 才算贴上（None = 自由落点）。判定输入只有
    /// 「内容矩形 + 内容所在屏」——此前贴在哪台屏哪条缘不是输入,沿边缘上下挪/换屏换边
    /// 因此都只按本屏重算（同一条接缝属于两台屏,归属由内容中心定;自由态表盘的跨缝
    /// 情形另由 `seam_landing_target` 补判）。
    fn dock_target(
        screen: &Screen,
    ...
    /// 自由态表盘贴边判定：**光标**（= 鼠标点击/拖动位置）到**内容所在屏**工作区
    /// 左/右缘的距离 ≤ `EDGE_EPS_LOGICAL`（几像素）**或已越过该缘** ⇒ 判贴上该缘。
    /// - 「越过」也算触缘（往外推过头是「更贴」的意图）——所以「把鼠标推到边上」
    ///   这种手势一定成立,不受抓取点偏移影响;光标推过接缝后仍算越过本屏的缘。
    /// - ⚠ **参照屏恒为内容所在屏**（光标只是测量点,不参与选屏）,即「判定与归位同屏」：
    ///   若把光标所在屏也当候选,光标一过接缝就判成隔壁屏左缘,归位按隔壁 scale 换算
    ///   ⇒ 窗口在自身 DPI 下 set_position/set_size 全部不生效,竖条留在原地不贴边。
    ///   **候选边缘只取本屏两缘,勿再加别的来源。**
    ...
    /// 自由态**跨缝补判**：拖动中窗口中心换过屏（= 系统已按「窗口过半」把窗口判给
    /// 新屏、DPI 也跟着切）时,**光标必然已经随拖动深入新屏**,离缝远超几像素的 eps
    /// ⇒ 光标口径整条失效：从接缝左侧拖到右侧,表盘会停在缝上既不贴边也不弹开;
    /// 反向却因「越过也算」的外侧无界而照常触发——同一个手势只有一半方向成立。
    ///
    /// 判据 = **主体是否完整显示**（与「拖入边缘时主体被截断才触发贴边」是同一条
    /// 逻辑的镜像）：主体跨过新屏近缘之后,只要它离新屏近缘仍 ≤ 几像素（含还压着缝、
    /// 尚未整体进入）就算贴住那条近缘;离缝超过几像素 ⇒ 主体已经**完整显示出来**
    /// （不再被屏缘截断）,就不必再贴边,该自由摆放。
    ///
    ...
    /// 拖动松手：只跑一次落点判定（自由态表盘 `cursor_dock_target`、已停靠与自由态竖条
    /// `dock_target`）,结果分三种走向——
    ...
    /// 自由态判定的测量点）——判定与归位同屏,跨屏拖动后不会出现「按旧屏归位」的
    ...
    fn on_exit() {
        ...
        // - 自由态**竖条** = 可见内容矩形 ≤ keep_tol（与已停靠同尺）——竖条的内容就是
        //   本体,不存在「提前触发」问题;按光标判时光标在竖条中部、离缘远大于 eps,屏内
        //   拖动又不满足跨缝补判的前置 crossed,贴近缘边也吸附不上;
        // - 已停靠 = 可见内容矩形 ≤ keep_tol（滞回：贴稳后要拖离约 12px 才脱边）。
    ...
    /// 把窗口的尺寸与位置一起重排 ⇒ 不重放就等于放弃归位（竖条会留在原地不贴边）。
    ...
    /// 一段可连发数十条）只换来静默 `REPLAY_DEBOUNCE_MS` 之后的**一次**写入——
    /// 「写 → 系统重估 → 再发消息 → 再写」的自激回路因此被时间隔断;单发消息只是
    /// 晚 40ms 重放,无感。
    ...
    ///
    /// 尺寸为什么写物理值：`set_size（LogicalSize)` 的换算在
    /// tao 内部用 `window_state.scale_factor` —— **消息驱动的缓存**。跨屏拖动松手时窗口
    /// 已归新屏（`GetDpiForWindow` 已报新值）,但 WM_DPICHANGED 还排在队列里 ⇒ 缓存仍是
    /// 旧屏 scale,目标屏 @1.5 的 56×116 逻辑会被写成 112×232 物理（应为 84×174）。
    /// 窗口因此比目标宽、内容压在接缝上,且「写错 ⇒ 尺寸越缝 ⇒ 系统重估 DPI ⇒
    ...
    /// 我们重放的拉锯（先缩后移、先移后缩都会在接缝某一侧闪烁,调整写入顺序按不住）。
    /// 原子写入把一次归位压缩成**一次**窗口矩形变更:
    ...
    /// 启动恢复的辅助判定（纯函数,单测覆盖）：求**记录窗口当时的形态**——只用于从
    /// 记录窗口位置换算内容原点,不决定启动形态（竖条只属于贴边,未贴边启动一律表盘）。
    ...
    /// - **未贴边**（有几何记录）⇒ **一律表盘**（竖条形态只属于贴边;上次退出是自由
    ///   竖条也回表盘,在竖条原位长大）,内容 + 阴影余量钳进工作区后精确写入;
    /// - **首次启动**（无记录）⇒ **展开态（表盘）** + 居中。
    pub fn restore_orb(
        window: &WebviewWindow,
        recorded: Option<(i32, i32, u32, u32)>,
    ...
    /// 系统把窗口框架整块重绘一遍——是划过容器时瞬间闪出原生窗体/标题栏
    ...
    /// 框架重绘;固定后剩下的旧式绘制入口由 `subclass_proc`
    ...
    /// 尊重 WS_EX_TRANSPARENT / HTTRANSPARENT」这类跨进程语义（只设样式位仍会被
    /// 遮挡——画布继续吃消息、挡住下层程序的悬停与点击）。恢复用
    /// `NULL` 区域（整窗）。
    /// ⚠ `SetWindowRgn` 成功后**系统接管 region 所有权**,不可再删;失败才自己删。
    /// `bredraw = 0`：不让系统**立即**重绘（立即重绘会造成「划过一次闪一下」）;
    /// 区域变化由合成器在下一帧按新形状重画,无中间帧。
    ...
    mod tests {
        ...
        /// 任一缘都判贴上——历史停靠屏/停靠边不参与（否则从别的屏拖到本屏右缘
        /// 会被判「离原停靠缘太远」而误弹表盘,跨接缝后会按旧屏归位）。
        #[test]
        ...
        /// 自由态表盘触发以**光标**（鼠标点击/拖动位置）为起点：光标到**内容所在屏**竖缘
        /// ≤ 几像素才触发,内容矩形不参与（否则表盘还没接触屏缘就触发）;越过该缘
        /// （含推过接缝）同样算触缘。
        ...
        /// 自由态**竖条**的落点口径 = 本体（与已停靠同尺）：手动折叠的竖条拖到离左屏
        /// 右缘 7px 松手——光标在竖条中部（离缘 19px > eps 6 不触缘）、屏内拖动又不满足
        /// 跨缝补判的前置 ⇒ 若按光标判两档全落空,竖条会被钳到离缘 24px 处;按本体判,
        /// 7 ≤ keep_tol(24) ⇒ 吸附贴缘。
        #[test]
        ...
        /// 跨缝补判：窗口中心换过屏 ⇒ 光标必然已深入新屏、光标口径失效,此时按
        /// 「**主体是否完整显示**」补判——主体还压着缝（尚未整体进入新屏）⇒ 贴;
        /// 离缝超过几像素 = 已完整显示 ⇒ 不贴（与「拖入边缘时主体被截断才触发贴边」
        /// 是同一条逻辑的镜像）。护栏针对「从接缝左侧拖到右侧,表盘停在缝上,既不贴边
        /// 也不弹开」。
        ...
        /// 贴边矩形（纯函数,dock 归位的唯一算法）：竖条**本体**距工作区缘
        ...
        /// 指针让出的滞回两档：让出判定带缓冲（离主体 > buf 才算离开）、
        ...
        /// 让出区域 = 主体 + 阴影余量（窗口坐标,物理像素）——展开态 @1.5:
        ...
        /// 意图 → **物理**矩形：逻辑 → 物理的换算取**目标屏自己的 scale**——
        /// 贴边按 work 找回的那台屏（窗口此刻可能还挂在另一台 DPI 不同的屏上）、
        /// 锚定按内容所在屏。写入据此走物理值,不受 tao 缓存滞后影响（缓存滞后期间
        /// 按逻辑值写会把 56×116 写成 112×232）。
        #[test]
