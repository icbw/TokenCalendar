//! 毛玻璃材质：走 DWM 做桌面级背板（CSS backdrop-filter 摸不到桌面）。直绑 window-vibrancy 0.6，
//! 其 Win32 版本门槛：SWCA=17763 / 未公开 Mica=22000 / host backdrop=22523。
//!
...
//!
//! 失败静默：window-vibrancy 对平台/版本不支持返回 Err，前端收到 Err 后回退关闭档；
//! host backdrop 路径内 DwmSetWindowAttribute 返回值被库忽略（不报 Err），版本支持性以 Err 分支为准。
//!
//! 主窗口圆角恒为 DWM ROUND（见 chrome.rs），材质开关只管背板；挂件材质态需要 DWM 圆角
//! （否则露方角背板），但挂件默认组合含 NCRP_DISABLED（与 ROUND 互斥），走
//! chrome.rs 的原子组合切换 apply_widget_material_chrome。
//! 挂件材质态的阴影无法去掉：NCRP 是阴影/边框/圆角的总开关，与 ROUND 裁切互斥；
//! tao set_shadow（false) 只影响尺寸 insets 计算，不是 DWM 阴影开关。
...
fn set_window_material_impl(
    ...
    // 主窗口圆角恒为 DWM ROUND（apply_main_chrome 启动装配），材质开关不翻转圆角；
    // 挂件材质态走 NCR 四属性原子组合切换（ROUND + 撤销 NCRP_DISABLED），Off 档完整还原。
    if label == "widget" {
