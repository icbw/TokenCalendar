...
//!
//! 四个窗口里只有悬浮球受害：主窗口 / 挂件 / 时间轴是可缩放窗口、布局按百分比
//! 流动，内容放大后仍自适应；悬浮球是**固定尺寸窗口**（580×310 / 56×116），
//! 表盘落点是固定 CSS 坐标，Rust 侧还按「CSS 像素 = 逻辑像素」硬算交互主体与
//! 「指针让出」区域（`SetWindowRgn`）——文本 150% 时表盘实际画在主体矩形的右下方
//! 且大 1.5 倍，光标离开后区域收紧 ⇒ 只剩表盘左上角一道弧。
//!
//! 处置（**文本缩放是全局设置，应用要跟随，不做抵消**）：本模块只负责
