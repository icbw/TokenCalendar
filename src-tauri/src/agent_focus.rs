//! 桌面窗口聚焦：亮起的项目行 / 条上项目名点击 → 把该会话所在 agent 的桌面窗口带到前台。
//!
...
//!   不拼接用户输入。
//!
//! 登记表取值来自各 agent 的实际安装（进程名 / 卸载表 / 安装目录）。Claude Code 是唯一有两种
...
// ---------- 双击会话跳转（open_agent_session） ----------
//
// 时间轴会话条双击 → 到该会话所在的 agent：窗口在 → 前置;不在 → 启动宿主（IDE 类带项目目录）;
// 宿主找不到 → 打开项目目录。精度是「对的应用 + 对的项目窗口」,不含会话级深链接。
//
// 启动红线：**只启动登记表内的宿主**——可执行文件名是本文件常量,路径只取自系统安装登记
// （App Paths / 卸载表 / 已安装的应用包）或正在运行的同名窗口进程,且文件名必须与登记名相等;
// 项目目录作为**独立参数**传入（不拼命令行字符串、不经 shell）,且只取库中会话自己的目录键;
// 前端只传 agent 键 + 会话 id,不传任何路径。
