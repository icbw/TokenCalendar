...
//! 标定系数换成「距上次读数大约消耗了百分之几」→ 达到阈值就排一次取数。这样取数
//! 频次贴着**实际调用量**走,而不是贴着钟表走。
//!
//! 三条规则：
//! - **复工即取**：本地 token 静默 `QUIET_SECS`（10 分钟）之后的第一笔,不看量直接排。
//!   理由不是「怕错过信号」,而是： 5h 是滚动窗口,离开期间旧用量不断过期,读数在**没有
//!   任何新消耗**的情况下也会漂,复工时贴边条上那个数多半已经不对; 标定样本要求两次读数
//!   间隔 ≤ 30 分钟,复工先取一轮等于把样本起点重置到工作开始的那一刻,否则本次工作的第一条
//!   样本会横跨整段空闲期直接作废（见 calib.rs）;
//! - **达阈即取**：预计消耗 ≥ 阈值（默认 5%,用户可调;开「余量低时收紧」后,5h 剩余
//!   ≤ `LOW_REMAINING_PCT` 时阈值减半）;
//! - **最小间隔**：两次取数**尝试**之间至少 `MIN_GAP_SECS`（60 秒）,一次巨量写入不会连打。
//!   起点是尝试时刻而非上次成功时刻——否则取数一旦连续失败,间隔判据就永远成立,
//!   每个采集轮都会再打一次。
//!
//! 账目在取到**新读数**时才清零（失败轮不清,下轮照样该取）;清零前的累计代价与分模型
//! 明细同时交给 calib.rs 落一条标定样本。没进展的轮（取数失败 / 判死 / 读数没推进）
//! 把应检时刻按 `backoff_secs` 指数推后（封顶 `MAX_BACKOFF_SECS`）：需求仍挂着,但不会
//! 在故障期间按采集频率空转重试,也不会因为「应检时刻永远是过去」而把触发来源恒判成
//! token（那会让兜底轮的零请求探测永久失效）。
...
/// 「离开」判据：本地 token 静默这么久即认为用户离开了
/// agent 工作。**同一个数同时是**「悬浮球减淡」（idle.rs 的待机,那边的安静起点由
/// `note_local_tokens` 经 `nudge_standby` 同步推进）与「复工即取一轮」的判据
/// ——它们本来就是同一件事,不该写两遍。
...
///
/// **由 2.0 改为 5.0**。理由是标定准了之后才看得清的一件事：
/// 这个阈值的单位是「5h 窗口的百分点」,而两个平台的窗口**值钱程度差一个数量级**
/// ——本机满 5h 窗 Claude Max ≈ $110、Codex ≈ $11〜14 的 API 等价用量。
/// 2% 对 Codex 只相当于两三毛钱的活就要打一次请求,按本机真实用量复算是日均 114 次、
/// 最忙一天上千次;5% 把它压到不到一半,而**读数新鲜度并不因此变差**——Codex 那一路
/// 现在由 `codex_rollout:update_snapshot` 从本地 rollout 零请求推进快照,
/// 阈值只决定「什么时候去问服务端要一次权威读数」。
///
/// 它是**全局**的（两个平台共用,设置页一个旋钮）：阈值的语义是「最多容忍多少百分点
/// 的显示滞后」,这个量本来就该按配额百分比而不是按钱来定。
...
fn tracks() -> std::sync::MutexGuard<'static, HashMap<Platform, Track>> {
    // poison 容忍（审计 P3-）：持锁线程 panic 后调度表仍可用。
...
mod tests {
    ...
    /// 比出厂预设（`PRIOR_SCALE` = 1 %/美元）刻意夸大 5 倍,好让夹具用整数 token 量
