...
fn fetch_one(adapters: &Adapters, platform: Platform) -> SubscriptionSnapshot {
    match platform {
        ...
        Platform::Claude => {
            if adapters.claude.is_dead(platform) {
                return dead_snapshot(platform);
            }
            match credentials::read_credential(platform) {
                Some(cred) => claude::fetch(&adapters.claude, cred),
                None => idle_snapshot(platform),
            }
        }
    }
}
