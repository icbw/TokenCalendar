...
- **项目维度**：Matrix 与 Insights 的维度切到 Project，按工作目录看 token；Project 维下可选
  Wait / Human 两项时间指标（并列展示，不相加）。没有工作目录的来源（CodeBuddy）归为 Unknown project。
...
> **升级提示**：统计口径变更的版本（如 0.5.7）首次启动会清空统计库，并从各 Agent 本地日志重新扫描，
> 首轮扫描完成前列表为空属正常。CodeBuddy 积分对账数据与项目管理设置保留；重扫只能还原各 Agent
> 本地仍保留的日志，需要保留旧数字时请先在「设置 → Data」备份。
