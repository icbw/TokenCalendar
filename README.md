...
- **0.9.6** —— **Insights 悬浮导航**:用量 / 定价 / 积分切换收进右上角悬浮按钮,不再独占一行;工具栏放不下时换行而非横向拖动;「用量趋势」更名为「用量」。不动数据。
- **0.9.5** —— **悬浮球不足 1% 不再显示 idle + 输入口径统一**:窗口已开始但用量不到 1% 时显示真实重置时刻并提示「已用不足 1%」;图表 / 矩阵分项改为输入（未命中缓存）/ 缓存读取 / 输出,价格面板用量表与价目表同列同名。不动数据。
- **0.9.4** —— **界面支持简体中文**:设置·通用新增语言（跟随系统 / English / 简体中文）,四个窗口与托盘菜单即时切换;日期按语言显示,紧凑数值保持 1.2M;中文默认周一开始。不动数据。
- **0.9.3** —— **周 hover 剩余消息数 + Claude 模型专属周限额**:悬浮球周额度悬停加各选中模型「约剩 / 满窗」条数;读取 Claude 逐模型周限额（如 Fable 独立的一条）,周提示与 Insights 显示其剩余,并按更紧的一条估算该模型的周条数。不动数据。
- **0.9.2** —— **剩余消息数按模型用法估**:常用模型按它自己最近的消息、最近没用的按它自己近 30 天的消息、几乎没用的才拿全部消息按它的价格估（Fable 5.1 满窗约 26 → 36 条,与历史窗口实测一致）;Insights 价格面板两张表底部说明收成一行。不动数据。
