...
export interface DesignPrefs {
  ...
  /** ：悬浮球（Orbit）总开关。默认关——新窗口形态不弹（与 window-state
   *  的 orb_visible 初值同口径）;开=设置页经 showOrb 显示 orb 窗口。写键经
   *  storage 桥广播,消费方在设置页。 */
  orbEnabled?: boolean
...
function sanitize(p: Partial<DesignPrefs>): Partial<DesignPrefs> {
  ...
  // 悬浮球总开关（布尔）+ 订阅轮询间隔（60〜1800 秒整）白名单清洗。
  if (p.orbEnabled !== undefined && typeof p.orbEnabled !== 'boolean') {
    delete p.orbEnabled
  }
