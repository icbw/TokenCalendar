...
import { collectorService, dataService, events, exportService, subscriptionService, updateService, windowService, type CredentialInfo, type ExportResult, type DataInfo, type SubscriptionSnapshot, type UpdateCheck, type UpdateProgress } from '../../services'
...
function SubscriptionsTab() {
  ...
  const toggleOrb = () => {
    ...
    windowService[orbVisible ? 'hideOrb' : 'showOrb']().catch(console.error)
  }
  ...
  return (
    <>
      ...
      <label className="setting-row">
        ...
        <input type="checkbox" checked={orbVisible} onChange={toggleOrb} />
      </label>
      <div className="setting-note">
    ...
    <>
      ...
      <label className="setting-row">
        ...
        <input
          ...
          onChange={(e) => setDesignPrefs({ autoUpdate: e.target.checked })}
        />
      </label>
      <div className="setting-note">
        Install new versions automatically at startup. The app closes while the installer runs.
      </div>
    </>
  )
}
