...
import { getDesignPrefs, setDesignPrefs, subscribeDesignPrefs, SIZE_PRESETS, RADIUS_PRESETS, SUBSCRIPTION_FETCH_PCT, MONTHLY_USD_MAX, ORB_MESSAGE_MODELS_MAX, applySubscriptionFetchPolicy, subscriptionFetchPct, subscriptionTightenLow, weekStartOf, type DesignPrefs, type SizePreset, type WeekStart } from './designPrefs'
...
import { useT, fmt, type MessageKey, type Translator } from '../../lib/i18n'
import './settings.css'

export type SettingsTab = 'general' | 'appearance' | 'projects' | 'data' | 'subscriptions' | 'about'

/** 本页字典键（模块常量里只存键,渲染时再取文案——切换语言即时生效）。 */
type SK = MessageKey<'settings'>

/** 页签（hint = hover 提示：一句话说明本页管什么）。 */
const TABS: { id: SettingsTab; label: SK; hint: SK }[] = [
  { id: 'general', label: 'tabGeneral', hint: 'tabGeneralHint' },
  { id: 'appearance', label: 'tabAppearance', hint: 'tabAppearanceHint' },
  { id: 'projects', label: 'tabProjects', hint: 'tabProjectsHint' },
  { id: 'data', label: 'tabData', hint: 'tabDataHint' },
  { id: 'subscriptions', label: 'tabSubscriptions', hint: 'tabSubscriptionsHint' },
  { id: 'about', label: 'tabAbout', hint: 'tabAboutHint' },
]

/** 色板项：label 是字典键。 */
type Swatch = { label: SK; hex: string | null }

/** 策展预定义色板：8 色，点击即用；首项=跟随 scheme 默认。 */
const WIDGET_SWATCHES: Swatch[] = [
  { label: 'swatchDefault', hex: null },
  { label: 'swatchSnow', hex: '#f8f9fb' },
  { label: 'swatchPaper', hex: '#f5efe2' },
  { label: 'swatchMist', hex: '#e8f0f2' },
  { label: 'swatchGraphite', hex: '#3a3f4a' },
  { label: 'swatchMidnight', hex: '#141b2d' },
  { label: 'swatchNavy', hex: '#1e3a5f' },
  { label: 'swatchForest', hex: '#1f3d2b' },
  { label: 'swatchPlum', hex: '#3b2a4d' },
]

/** 主界面色板：中性系 + 与挂件同款的深色锚点。
 * 首项=跟随 scheme 默认（A 圆点）。 */
const MAIN_SWATCHES: Swatch[] = [
  { label: 'swatchDefault', hex: null },
  { label: 'swatchSnow', hex: '#f8f9fb' },
  { label: 'swatchMist', hex: '#e8f0f2' },
  { label: 'swatchFog', hex: '#dde4ec' },
  { label: 'swatchGraphite', hex: '#3a3f4a' },
  { label: 'swatchMidnight', hex: '#141b2d' },
  { label: 'swatchSlate', hex: '#2b3648' },
  { label: 'swatchNavy', hex: '#1e3a5f' },
  { label: 'swatchEspresso', hex: '#2e2620' },
]

/** 时间轴主题色色板：强调色而非底色——中等饱和度,亮 / 暗两主题下与卡片底混色都可读。
 * 首项 = 跟随全局 accent。 */
const TIMELINE_ACCENT_SWATCHES: Swatch[] = [
  { label: 'swatchDefault', hex: null },
  { label: 'swatchIndigo', hex: '#6366f1' },
  { label: 'swatchViolet', hex: '#8b5cf6' },
  { label: 'swatchRose', hex: '#e11d48' },
  { label: 'swatchAmber', hex: '#d97706' },
  { label: 'swatchEmerald', hex: '#059669' },
  { label: 'swatchTeal', hex: '#0d9488' },
  { label: 'swatchSky', hex: '#0284c7' },
  { label: 'swatchSlate', hex: '#64748b' },
]
    ...
    <div className="settings-page">
      ...
      <div className="settings-toolbar">
        <div className="settings-toolbar-inner">
          <button className="settings-back" onClick={onBack} title={t('backToMatrix')} aria-label={t('backToMatrix')}>
            <BackIcon />
            <span>{t('backMatrix')}</span>
          </button>
...
/** 语言三档：System = 删键（跟随系统）;English / 简体中文用各自语言显示,不随界面语言翻译。 */
const LANGUAGE_OPTIONS: { value: 'system' | 'en' | 'zh-CN'; native: string | null }[] = [
  { value: 'system', native: null },
...
function GeneralTab() {
    ...
    autostartService
      ...
      .then((r) => {
        if (r) {
          ...
          setAutostartNote(t('autostartFailed'))
          autostartService.getAutostart().then((v) => v !== null && setAutostart(v)).catch(console.error)
        }
      })
      .catch(console.error)
  ...
  return (
    <>
      <div className="setting-section">{t('sectionInterface')}</div>
      <div className="setting-block">
        <div className="setting-row">
          <span title={t('languageHint')}>{t('language')}</span>
          <div className="setting-seg">
            {LANGUAGE_OPTIONS.map((o) => (
              <button
                key={o.value}
                className={`setting-seg-btn${(design.locale ?? 'system') === o.value ? ' is-active' : ''}`}
                title={o.native ?? t('langSystemHint')}
                lang={o.value === 'system' ? undefined : o.value}
                onClick={() => setDesignPrefs({ locale: o.value === 'system' ? undefined : o.value })}
              >
                {o.native ?? t('langSystem')}
              </button>
            ))}
          </div>
        </div>
      ...
      <div className="setting-section">{t('sectionStartup')}</div>
      <div className="setting-block">
        <ToggleRow
      ...
      <div className="setting-section">{t('sectionCollection')}</div>
      <div className="setting-block">
        <ToggleRow
          label={t('pauseCollection')}
          title={t('pauseCollectionHint')}
          checked={paused}
          onChange={togglePause}
        />
        <div className="setting-row">
          <span title={t('collectEveryHint')}>
            {t('collectEvery')}
          </span>
          <div className="setting-seg">
            {COLLECT_INTERVAL_CHOICES.map(({ secs, label }) => (
              <button
                ...
                title={t(secs === 30 ? 'scanEveryDefault' : 'scanEvery', { label })}
      ...
      <div className="setting-section">{t('sectionMatrix')}</div>
      <div className="setting-block">
        <div className="setting-row">
          <span title={t('maxRowsHint')}>
            {t('maxRows')}
          </span>
          <div className="setting-seg">
            {[8, 12, 15, 20, 0].map((n) => (
              <button
                ...
                title={n === 0 ? t('noLimit') : t('showUpToRows', { n })}
        ...
        <div className="setting-row">
          ...
          <div className="setting-seg">
            {(['sunday', 'monday'] as WeekStart[]).map((d) => (
              <button
                key={d}
                className={`setting-seg-btn${weekStartOf(design) === d ? ' is-active' : ''}`}
                title={t(d === 'sunday' ? 'weeksStartSunday' : 'weeksStartMonday')}
                onClick={() => setDesignPrefs({ weekStart: d })}
              >
                {t(d === 'sunday' ? 'sunday' : 'monday')}
              </button>
            ))}
          </div>
        </div>
      ...
      <div className="setting-section">{t('sectionTimeline')}</div>
      <div className="setting-block">
        <ToggleRow
          label={t('showTimeline')}
          title={t('showTimelineHint')}
          checked={timelineVisible}
          onChange={toggleTimeline}
        />
        <div className="setting-row">
          <span title={t('pastDaysHint')}>{t('pastDays')}</span>
          <div className="setting-seg">
            {[3, 7, 15, 30].map((n) => (
              <button
                key={n}
                className={`setting-seg-btn${(design.timelinePastDays ?? 7) === n ? ' is-active' : ''}`}
                title={t('showDaysBefore', { n })}
        ...
        <div className="setting-row">
          <span title={t('pastSessionsHint')}>{t('pastSessions')}</span>
          <div className="setting-seg">
            {[1, 2, 3, 5].map((n) => (
              <button
                key={n}
                className={`setting-seg-btn${(design.timelinePastSessions ?? 1) === n ? ' is-active' : ''}`}
                title={t(n === 1 ? 'showPastSessions_one' : 'showPastSessions_other', { n })}
        ...
        <div className="setting-row">
          <span title={t('todaySessionsHint')}>{t('todaySessions')}</span>
          <div className="setting-seg">
            {[3, 5, 8, 12].map((n) => (
              <button
                key={n}
                className={`setting-seg-btn${(design.timelineTodaySessions ?? 5) === n ? ' is-active' : ''}`}
                title={t('showTodaySessions', { n })}
        ...
        <div className="setting-row">
          <span title={t('pickByHint')}>{t('pickBy')}</span>
          <div className="setting-seg">
            {(
              [
                { v: 'latest', label: 'pickLatest', hint: 'pickLatestHint' },
                { v: 'earliest', label: 'pickEarliest', hint: 'pickEarliestHint' },
                { v: 'longest', label: 'pickLongest', hint: 'pickLongestHint' },
              ] as const
            ).map((o) => (
              <button
                key={o.v}
        ...
        <ToggleRow
          label={t('newestFirst')}
          title={t('newestFirstHint')}
          checked={design.timelineReverse ?? false}
          onChange={(v) => setDesignPrefs({ timelineReverse: v })}
        />
        <div className="setting-row">
          <span title={t('autoFoldHint')}>{t('autoFold')}</span>
          <div className="setting-seg">
            {[0, 5, 30, 60, 300].map((n) => (
              <button
                key={n}
                className={`setting-seg-btn${(design.timelineAutoStripSecs ?? 0) === n ? ' is-active' : ''}`}
                title={n === 0 ? t('neverFold') : n < 60 ? t('foldAfterSecs', { n }) : t(n === 60 ? 'foldAfterMin_one' : 'foldAfterMin_other', { n: n / 60 })}
                onClick={() => setDesignPrefs({ timelineAutoStripSecs: n })}
              >
                {n === 0 ? t('off') : n < 60 ? `${n}s` : `${n / 60}m`}
              </button>
            ))}
          </div>
        </div>
        <div className="setting-row">
          <span title={t('futureDaysHint')}>{t('futureDays')}</span>
          <div className="setting-seg">
            {[0, 3, 7, 15].map((n) => (
              <button
                key={n}
                className={`setting-seg-btn${(design.timelineFutureDays ?? 7) === n ? ' is-active' : ''}`}
                title={n === 0 ? t('noFutureDays') : t('showDaysAfter', { n })}
      ...
      <div className="setting-section">{t('sectionWidget')}</div>
      <div className="setting-block">
        <div className="setting-row">
          <span title={t('widgetSizeHint')}>{t('widgetSize')}</span>
          <div className="setting-seg">
            {(['large', 'medium', 'small'] as SizePreset[]).map((p) => (
              ...
              >
                {t(p === 'large' ? 'sizeLarge' : p === 'medium' ? 'sizeMedium' : 'sizeSmall')}
              </button>
            ))}
          </div>
        </div>
...
function AppearanceTab() {
  ...
  useEffect(() => {
    ...
    const onDown = (e: MouseEvent) => {
      ...
      if (colorRowRefs[pickerFor].current?.contains(target)) return
      if (popoverRefs[pickerFor].current?.contains(target)) return
      setPickerFor(null)
    }
  ...
  const renderColorField = (slot: ColorSlot, presets: Swatch[]) => {
    const s = slots[slot]
        ...
        <div ref={s.popoverRef} className={`color-popover${pickerFor === slot ? ' is-open' : ''}`} aria-hidden={pickerFor !== slot}>
          ...
          <div className="color-popover-input">
            <HexColorInput
              ...
              prefixed
            />
            <button className="setting-btn" onClick={() => s.set(undefined)} title={t('clearCustomColor')}>
              {t('auto')}
            </button>
          </div>
        </div>
  ...
  return (
    <>
      <div className="setting-section">{t('sectionCornerRadius')}</div>
      <div className="setting-block">
        <div className="setting-row">
          <span title={t('radiusSchemeHint')}>
            {t('radiusScheme')}
          </span>
          <div className="setting-seg">
            {(['small', 'medium', 'large'] as const).map((r) => (
              <button
                ...
                title={t('radiusCorners', { n: RADIUS_PRESETS[r].panel })}
                onClick={() => setDesignPrefs({ radiusScheme: r })}
              >
                {t(r === 'small' ? 'radiusSmall' : r === 'medium' ? 'radiusMedium' : 'radiusLarge', { n: RADIUS_PRESETS[r].panel })}
              </button>
            ))}
          </div>
        </div>
      ...
      <div className="setting-section">{t('sectionWidgetAppearance')}</div>
      <div className="setting-block">
        <div className="setting-row">
          <span title={t('cardColorHint')}>
            {t('cardColor')}
          </span>
          <span className="setting-value">{custom ? custom.toUpperCase() : t('autoFollowsSystem')}</span>
        </div>
        {renderColorField('card', WIDGET_SWATCHES)}

        <div className="setting-row">
          <span title={t('bgOpacityHint')}>{t('bgOpacity')}</span>
          <input
            type="range"
        ...
        <div className="setting-row">
          <span title={t('heatmapOpacityHint')}>{t('heatmapOpacity')}</span>
          <input
            type="range"
            title={t('dragToAdjust')}
            min={0.2}
        ...
        <div className="setting-preview" style={{ background: previewBg, borderColor: custom ? theme?.cardBorder : undefined }}>
          <span className="setting-preview-title" style={{ color: custom ? theme?.text : undefined }}>{t('previewTitle')}</span>
      ...
      <div className="setting-section">{t('sectionMainInterface')}</div>
      <div className="setting-block">
        <div className="setting-row">
          <span title={t('titlebarColorHint')}>{t('titlebarColor')}</span>
          <span className="setting-value">{design.titlebarBg ? design.titlebarBg.toUpperCase() : t('autoFollowsSystem')}</span>
        </div>
        {renderColorField('titlebar', MAIN_SWATCHES)}
        <div className="setting-row">
          <span title={t('titlebarOpacityHint')}>{t('titlebarOpacity')}</span>
          <input
            type="range"
            title={t('dragToAdjust')}
            min={0.2}
        ...
        <div className="setting-row">
          <span title={t('mainPanelColorHint')}>{t('mainPanelColor')}</span>
          <span className="setting-value">{design.panelBg ? design.panelBg.toUpperCase() : t('autoFollowsSystem')}</span>
        </div>
        {renderColorField('panel', MAIN_SWATCHES)}

        <div className="setting-row">
          <span title={t('borderColorHint')}>{t('borderColor')}</span>
          <span className="setting-value">{design.borderColor ? design.borderColor.toUpperCase() : t('autoFollowsSystem')}</span>
        </div>
      ...
      <div className="setting-section">{t('sectionTimelineAppearance')}</div>
      <div className="setting-block">
        <div className="setting-row">
          <span title={t('themeColorHint')}>{t('themeColor')}</span>
          <span className="setting-value">{design.timelineAccent ? design.timelineAccent.toUpperCase() : t('autoFollowsSystem')}</span>
        </div>
        {renderColorField('timeline', TIMELINE_ACCENT_SWATCHES)}
        <div className="setting-row">
          <span title={t('windowStyleHint')}>{t('windowStyle')}</span>
          <div className="setting-seg">
            {(['shadow', 'flat'] as const).map((v) => (
              <button
                key={v}
                className={`setting-seg-btn${(design.timelineWindowStyle ?? 'shadow') === v ? ' is-active' : ''}`}
                title={t(v === 'shadow' ? 'styleShadowHint' : 'styleFlatHint')}
                onClick={() => setDesignPrefs({ timelineWindowStyle: v })}
              >
                {t(v === 'shadow' ? 'styleShadow' : 'styleFlat')}
              </button>
            ))}
          </div>
        </div>
        <ToggleRow
          label={t('colorCellsByUsage')}
          title={t('colorCellsByUsageHint')}
        ...
        {TIMELINE_ALPHA_ROWS.map((r) => (
          <div key={r.key} className="setting-row">
            <span title={t(r.hint)}>{t(r.label)}</span>
            <input
              type="range"
              title={t('dragToAdjust')}
              min={r.min}
...
const TIMELINE_ALPHA_ROWS: { key: 'timelineBgAlpha' | 'timelineBarAlpha' | 'timelineCellAlpha'; label: SK; hint: SK; min: number; def: number }[] = [
  { key: 'timelineBgAlpha', label: 'panelBgOpacity', hint: 'panelBgOpacityHint', min: 0, def: TIMELINE_BG_ALPHA },
  { key: 'timelineBarAlpha', label: 'topBarOpacity', hint: 'topBarOpacityHint', min: 0.2, def: TIMELINE_BAR_ALPHA },
  { key: 'timelineCellAlpha', label: 'sessionCellOpacity', hint: 'sessionCellOpacityHint', min: 0.2, def: TIMELINE_CELL_ALPHA },
]

/* ---------------- 材质三档（两窗口共用行控件） ---------------- */

const MATERIAL_OPTIONS: { label: SK; value: 'mica' | 'acrylic' | null; hint: SK }[] = [
  { label: 'off', value: null, hint: 'materialOffHint' },
  { label: 'materialMica', value: 'mica', hint: 'materialMicaHint' },
  { label: 'materialAcrylic', value: 'acrylic', hint: 'materialAcrylicHint' },
]
...
function MaterialRow({
  ...
  return (
    <div className="setting-row">
      <span title={title}>{t('glassMaterial')}</span>
      <div className="setting-seg">
...
function DataTab() {
  ...
  const doExport = (format: 'csv' | 'json') => {
    ...
    p.then((res: ExportResult | null) => {
      if (res) setExportResult(t('exportedRows', { format: format.toUpperCase(), rows: res.rows, path: res.path }))
      else setExportResult(t('exportFailed'))
    })
      .catch((e: unknown) => {
        console.error('[export]', e)
        setExportResult(t('exportFailed'))
      })
  ...
  const run = async (kind: 'migrate' | 'backup' | 'restore') => {
    ...
    try {
      if (kind === 'migrate') {
        ...
        setNotice(r ? t('restartToApply', { msg: r }) : t('migrationFailed'))
      } else if (kind === 'backup') {
        const r = await dataService.backupData(bakTarget.trim())
        setNotice(r ? t('backupFinished', { path: r.path }) : t('backupFailed'))
      } else {
        const r = await dataService.restoreData(resTarget.trim())
        setNotice(r ? t('restartToApply', { msg: r }) : t('restoreFailed'))
      }
    } finally {
      setBusy(null)
    }
  ...
  const dirPick = { migrate: t('pickMigrate'), backup: t('pickBackup'), restore: t('pickRestore') }

  return (
    <>
      <div className="setting-section">{t('sectionStorage')}</div>
      <div className="setting-block">
        {info ? (
          <div className="setting-note">
            {t('storageRoot', { root: info.root })}
            {info.fell_back ? t('storageFellBack') : ''}
            {info.custom_root ? <><br />{t('storageCustom', { root: info.custom_root })}</> : null}
            <br />{t('storageStats', { size: fmtBytes(info.db_bytes), n: info.exports_count })}
          </div>
        ) : (
          <div className="setting-note">{t('storageUnavailable')}</div>
        )}
        <div className="setting-actions">
          <button
            className="setting-btn"
            onClick={() => void dataService.openDataDir()}
            title={t('openDataFolderHint')}
          >
            {t('openDataFolder')}
          </button>
        </div>
      ...
      <div className="setting-section">{t('sectionCreditCard')}</div>
      <div className="setting-block">
        <div className="setting-note">
          {t('creditNote')}
        </div>
        <ToggleRow
          label={t('showCreditCard')}
          title={t('showCreditCardHint')}
          checked={design.insightsCredit}
          onChange={(v) => setDesignPrefs({ insightsCredit: v })}
        />
      </div>

      <div className="setting-section">{t('sectionMigrate')}</div>
      <div className="setting-block">
        <div className="setting-note">
          {t('migrateNote')}
        </div>
        <div className="setting-actions">
          ...
          <button className="setting-btn" onClick={() => void pickInto(setMigTarget, dirPick.migrate)} title={t('pickFolder')}>{t('browse')}</button>
          <button
            className="setting-btn"
            ...
            title={t('migrateHint')}
          >
            {busy === 'migrate' ? t('migrating') : t('migrate')}
          </button>
        </div>
      </div>

      <div className="setting-section">{t('sectionBackup')}</div>
      <div className="setting-block">
        <div className="setting-note">{t('backupNote')}</div>
        <div className="setting-actions">
          <input className="setting-input" placeholder="E:\Backup\TokenCalendar" value={bakTarget} onChange={(e) => setBakTarget(e.target.value)} />
          <button className="setting-btn" onClick={() => void pickInto(setBakTarget, dirPick.backup)} title={t('pickFolder')}>{t('browse')}</button>
          <button
            className="setting-btn"
            ...
            title={t('backupHint')}
          >
            {busy === 'backup' ? t('backingUp') : t('backupNow')}
          </button>
        </div>
      </div>

      <div className="setting-section">{t('sectionRestore')}</div>
      <div className="setting-block">
        <div className="setting-note">
          {t('restoreNote')}
        </div>
        <div className="setting-actions">
          ...
          <button className="setting-btn" onClick={() => void pickInto(setResTarget, dirPick.restore)} title={t('pickFolder')}>{t('browse')}</button>
          <button
            className="setting-btn"
      ...
      <div className="setting-section">{t('sectionExportMonth', { month })}</div>
      <div className="setting-block">
        <div className="setting-actions">
          <button
            className="setting-btn"
...
function SubscriptionsTab() {
  ...
  const commitFetchPct = (draft: string) => {
    ...
    if (!Number.isFinite(n) || n < min || n > max || !Number.isInteger(n / step)) {
      setNotice(t('fetchThresholdInvalid', { min, max, step }))
  ...
  const doBind = (platform: CredentialInfo['platform']) => {
    ...
    subscriptionService.bind(platform).then(() => {
      setNotice(t('platformBound', { platform }))
    }).catch((e) => {
      setNotice(String(e))
      console.error(e)
    })
  ...
  const doUnbind = (platform: CredentialInfo['platform']) => {
    ...
    subscriptionService.unbind(platform).then(() => {
      setNotice(t('platformUnbound', { platform }))
    }).catch((e) => {
      setNotice(String(e))
      console.error(e)
    })
  ...
  return (
    <>
      <div className="setting-section">{t('sectionFloatingOrb')}</div>
      <div className="setting-block">
        <ToggleRow
          label={t('showOrb')}
          title={t('showOrbHint')}
        ...
        <div className="setting-row">
          <span title={t('fetchThresholdHint')}>
            {t('fetchThreshold')}
          </span>
          <span>
            <input
        ...
        {estimator.length > 0 ? (
          <div className="setting-note">
            {estimator.map((e) => (
              <div
                key={e.platform}
                title={t('estUsedSinceReading', { pct: e.est_pct_since_fetch.toFixed(1) })}
              >
                {e.platform === 'claude' ? 'Claude' : 'Codex'} ·{' '}
                {e.calibrated ? t('calibrated') : t('calibrating')}
                {t(e.pairs === 1 ? 'samples_one' : 'samples_other', { n: e.pairs })}
                {/* 出厂预设是在某一档上、再按官方限额比折到用户这一档的 → 没校准前可能偏一点
                    （-10）。已校准后不再相关,不显示。*/}
                {!e.calibrated ? (
                  <span>
                    {t('presetNote', { plan: e.platform === 'claude' ? 'Max 5x' : 'Plus' })}
                  </span>
                ) : null}
                {/* 收割留存的本地读数条数（Claude = 桌面端采样;Codex = 会话 rollout 里的
                    rate_limits;0 / 缺字段时整段不显示）——两个源自己都会滚掉旧数据,
                    这个数越过源的保留线继续涨就是密度在累积。*/}
                {e.desktop_samples ? t('localReadingsKept', { n: e.desktop_samples }) : ''}
                ...
                {e.foreign && e.foreign.unexplained > 0 ? (
                  <span
                    ...
                    title={t('foreignHint', { n: e.foreign.unexplained, windows: e.foreign.windows, pct: e.foreign.unexplained_pct.toFixed(1) })}
                  >
                    {t('foreignNote', { n: e.foreign.unexplained, windows: e.foreign.windows, hours: e.foreign.window_hours, pct: e.foreign.unexplained_pct.toFixed(1) })}
                  </span>
                ) : null}
              </div>
            ))}
      ...
      <div className="setting-section">{t('sectionPlatforms')}</div>
      <div className="setting-block">
        {scan.map((info) => {
                ...
                <small>
                  {!info.present
                    ...
                    : !info.parseable
                      ...
                      : `${info.account_hint ?? t('localAccount')} · ${statusText}`}
                </small>
              </span>
              <div className="setting-seg">
                {bound ? (
      ...
      {(['codex', 'claude'] as const).some(boundOf) ? (
        <>
          <div className="setting-section">{t('sectionOrbMessages')}</div>
          <div className="setting-block">
      ...
      <div className="setting-section">{t('sectionPlanFees')}</div>
      <div className="setting-block">
...
function MessageModelsRow({ platform, value }: {
  ...
  const perMsg = (key: string) => {
    ...
    return r
      ? t('msgPerWindow', { name: r.display_name, n: Math.round(100 / r.pct_per_turn) })
      : t('msgNotEnough')
  }
  return (
    <div className="setting-row">
      <span title={t('msgModelsHint', { label })}>{label}</span>
      <div className="setting-seg is-wrap">
        <button
          className={`setting-seg-btn${value === undefined ? ' is-active' : ''}`}
          title={
            budget?.main_model
              ? t('msgAutoHintModel', { model: shortModelName(budget.main_model) })
              : t('msgAutoHint')
          }
...
function MonthlyFeeRow({ platform, value, onNotice }: {
  ...
  const commit = () => {
    ...
    if (!Number.isFinite(n) || n <= 0 || n > MONTHLY_USD_MAX) {
      onNotice(t('monthlyFeeInvalid', { max: fmt.number(MONTHLY_USD_MAX) }))
  ...
  return (
    <div className="setting-row">
      <span title={t('monthlyFeeHint', { label })}>{t('monthlyFee', { label })}</span>
      <span>
        <input
          className="setting-num"
          type="number"
          aria-label={t('monthlyFee', { label })}
          placeholder="—"
          value={draft}
          min={0}
          step={1}
          ...
          onKeyDown={(e) => e.key === 'Enter' && (e.target as HTMLInputElement).blur()}
        />
        <span className="setting-unit">{t('usdPerMonth')}</span>
      </span>
    </div>
  )
}
...
function AboutTab() {
  ...
  const doInstall = async (install: (cb: (p: UpdateProgress) => void) => Promise<void>) => {
    ...
    try {
      ...
      setNote(t('installerLaunched'))
    } catch (e) {
      setNote(t('installFailed', { error: e instanceof Error ? e.message : String(e) }))
      setInstalling(false)
    }
  }
  ...
  const statusText = (): string => {
    ...
    if (installing) {
      if (!progress || progress.total === 0) return t('downloading')
      const pct = Math.min(100, Math.round((progress.downloaded / progress.total) * 100))
      return progress.finished ? t('downloadComplete') : t('downloadingPct', { pct })
    }
    // 已预下载的版本不比检查结果旧时,以「可直接安装」为准
    if (ready && (result?.status !== 'available' || result.version === ready.version)) {
      return version ? t('readyToInstallCurrent', { version: ready.version, current: version }) : t('readyToInstall', { version: ready.version })
    }
    if (!result) return t('servedFrom')
    switch (result.status) {
      case 'unavailable':
        return t('unavailable')
      case 'up-to-date':
        return t('upToDate', { current: result.current })
      case 'available':
        return result.canInstall
          ? t('available', { version: result.version, current: result.current })
          : t('availableDev', { version: result.version, current: result.current })
      case 'error':
  ...
  return (
    <>
      <div className="setting-section">{t('sectionUpdates')}</div>
      <div className="setting-block">
        <div className="setting-row">
          <span title={t('versionHint')}>{t('version')}</span>
          <button
            className="setting-btn"
            onClick={() => {
              updateService.openReleasesPage().catch(console.error)
            }}
            title={t('openReleasesHint')}
          >
            {version ? `v${version}` : '—'}
          </button>
        </div>
        ...
        <div className="setting-actions">
          ...
          {installAction ? (
            <button
              ...
              title={
                installDownloaded
                  ? t('installDownloadedHint')
                  : t('downloadInstallHint')
              }
            >
              {installDownloaded ? t('install') : t('downloadInstall')}
            </button>
          ) : null}
        </div>
        ...
        <ToggleRow
          label={t('checkAtStartup')}
          title={t('checkAtStartupHint')}
...
/** 额度窗口种类 → 显示名（英文照印原值;中文 5h → 5 小时、7d → 每周、7d_fable → Fable 每周）。 */
function windowKindLabel(t: Translator<'settings'>, kind: string): string {
  if (kind === '5h') return t('windowKind5h', { kind })
  if (kind === '7d') return t('windowKind7d', { kind })
  const m = /^7d_(.+)$/.exec(kind)
  if (m) return t('windowKindModel', { kind, model: m[1].charAt(0).toUpperCase() + m[1].slice(1) })
  return kind
}
