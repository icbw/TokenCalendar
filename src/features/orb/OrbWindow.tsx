...
// - 展开态 = 周额度环 + 中央 5h 表盘（150px 正圆,≈ 旧版「内圈」量级）：
//   环内四件内容 = 5h 剩余大数（**只显示剩余,不带总量**）/ 5h 重置时刻
//   （HH:MM 直接读数,非文字标签）/ ● + 套餐名一行（状态点前置）/ 周重置
//   倒计时（纯文字）。功能按钮移出外圈,在右侧排成一列悬挂按钮。
 ...
 * ⇒ 内容偏移 = （215, 80),即 padL + 75 = W/2、padT + 75 = H/2（表盘 150×150,
 * 视觉中心在内容块左上 （75,75) 处）。右侧画布因此仍是 190（按钮列提示照旧走得下）。
 * ⚠ 与 Rust 的 EXPANDED_W/H_LOGICAL、EXPANDED_PAD_L/T_LOGICAL 必须同源。 */
const EXPANDED_SIZE = { w: 580, h: 310 }

/** 展开态交互主体尺寸（内容区 175×150）与它在窗口内的左上偏移——与 Rust 穿透
...
const EXPANDED_INNER = { w: 175, h: 150 }
/** 内容在窗口内的左上偏移：**左右/上下都围绕表盘视觉中心对称**（见 EXPANDED_SIZE
 * 注释）——215 + 75 = 580/2,80 + 75 = 310/2。与 Rust `EXPANDED_PAD_L/T_LOGICAL`
 * 同源（CSS 里 16/20 → 215/80 一并改）。 */
const EXPANDED_PAD = { l: 215, t: 80 }

/** 表盘几何（viewBox 150×150 定值,外层 CSS 不缩放）：
 * 壳体 r74 / 周额度环 r70（内缩 4px 留出一圈玻璃边）/ 中央 5h 表盘 92px。 */
const DIAL_VIEW = 150
const DIAL_R = 74
const RING_R = 70
...
export default function OrbWindow() {
  ...
  // 位置：环/表盘这类大面积区域跟光标（12px 偏移,越界翻转+钳制）;19px 悬挂
  ...
  /** 悬挂按钮（19px 小钮）：锚按钮中心而非光标——贴着按钮边缘时提示会跟着抖。 */
  ...
  const weeklyTip: TipContent = w7d
    ? {
        label: 'Weekly quota',
        lines: [
    ...
    : statusTip
  const fiveTip: TipContent = w5h
    ? {
        label: '5-hour quota',
        lines: [`${pctText(remain5h)} / 100 left`, ...(reset5h ? [`resets at ${reset5h}`] : [])],
      }
    : statusTip
  const resetTip: TipContent = weekStamp
    ? { label: 'Weekly reset', lines: [weekStamp] }
    : statusTip
  // 提示动作（悬挂按钮：锚按钮中心,19px 小钮上跟光标会抖）
  ...
  const onDialMove = useCallback(
    (e: React.MouseEvent) => {
      ...
      // 命中半径取 min（宽, 高)/2：gauge-wrap 是整幅宽（150）的居中容器,
      // 真正的表盘圆只有 92px（wrap 高度随内容 → 高度即直径）
  ...
  return (
    ...
    >
      ...
      {/* ---- 展开态：周额度环 + 中央 5h 表盘 ----*/}
      {/* 形态基线：外圈 = 周额度进度环,直径 150 ≈ 旧版「内圈」（中央 5h 表盘）
          的量级。环内四件内容（⑱）：5h 剩余大数（只显示剩余,不带总量）/
          5h 重置时刻 HH:MM / ● + 套餐名一行（状态点前置,无 Active 文字）/
          周重置倒计时（纯文字,不加粗无背景）。
      ...
      >
        ...
        >
          ...
          {/* 中央 5h 主表盘（唯一读数：剩余大数 + 重置时刻）*/}
          <div className="orb-orb-gauge-wrap" ref={gaugeRef}>
            <FiveGauge pct={remain5h} resetAt={reset5h} />
          </div>

          {/* 底部两行（⑱）： 状态点 + 套餐名（点前置,不再有 Active 文字——
              四态仅由点色表达,完整文案落 hover 提示） 周重置倒计时（纯文字）。
              两张文本各自挂提示（㉕）：套餐行 → 状态文案（带四态点）;
              重置行 → 绝对重置时刻（与可见倒计时互补）。
          ...
          >
            <span className={`orb-card-dot is-${hint.level}`} aria-hidden="true" />
...
function FiveGauge({ pct, resetAt }: { pct: number | null; resetAt: string | null }) {
  const r = 46 // viewBox 100 固定半径,外层 CSS 缩放到 92px
  ...
  return (
    <div className={`orb-gauge${valid ? '' : ' is-empty'}`}>
      ...
      <div className="orb-gauge-center">
        <span className="orb-gauge-num">{valid ? Math.round(shown) : '—'}</span>
        <span className="orb-gauge-reset">{resetAt ?? '—'}</span>
      </div>
    </div>
  )
}
