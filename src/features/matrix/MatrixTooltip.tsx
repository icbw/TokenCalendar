...
interface MatrixTooltipProps {
   ...
   * 主窗口无浮层，不传即保持原行为。 */
  topReserve?: number
  /** 紧凑单行变体：两行浮层高 ≈52px，在挂件
   * 矮窗里等于压住 2〜3 行格子——往上/往下滑到下一行时看不到自己 hover 的
   * 行。紧凑变体 = 单行「日期 · 读数」，高 ≈23px（一行格子），只压相邻一行，
   * hover 的那行永远可。主窗口空间充裕，保持完整两行格式。 */
  compact?: boolean
}
 ...
 * 旧实现按「左边距」钳制（left = max（margin, ...)），最左几列的 tooltip
 * 左半截被窗口边切掉。
 * 2. **垂直**：旧实现只判断锚点上方够不够，从不检查目标落位的下边是否出窗
 * ——挂件矮窗（92/134/155 三档）里中间几行翻到下方后正好被窗口底边裁掉。
 * 现按「上方优先（避开顶部保留区）→ 下方（须完整放得下）→ 两侧都放不下
 * 时取空间更大的一侧，再钳进窗口」定序。
 * 结论：**不需要放大窗口**——窗口 = 卡片是贴边方案与材质态的硬约束，出界是
 * 定位数学问题，不是容器尺寸问题。 */
function placeFor(
  anchor: HTMLElement,
  el: HTMLElement,
