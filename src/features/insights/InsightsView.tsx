...
function Seg<T extends string | number>({ value, options, onChange }: {
  ...
  return (
    <div className="toolbar-group">
      {options.map((o) => (
        <button key={String(o.v)} className={`seg${value === o.v ? ' is-active' : ''}`} onClick={() => onChange(o.v)}>
          {o.label}
        </button>
      ))}
    </div>
  ...
  return {
    toolbar: (
      <header className="insight-toolbar">
        ...
        <Seg
          value={kind}
          options={[{ v: 'line' as ChartKind, label: 'Lines' }, { v: 'stack' as ChartKind, label: 'Stack' }]}
          onChange={setKind}
        />
        <Seg
          value={rangeDays}
          options={([7, 30, 90] as RangeDays[]).map((d) => ({ v: d, label: RANGE_LABELS[d] }))}
          onChange={(v) => { setRangeDays(v); setFilterKey('') }}
        />
        <Seg
          value={effectiveBucket}
          options={[{ v: 'day' as Bucket, label: 'Day' }, { v: 'hour' as Bucket, label: 'Hour' }]}
          onChange={setBucket}
        />
        <Seg
          value={dimension}
          options={[
            { v: 'model' as Dimension, label: 'Model' },
            { v: 'agent' as Dimension, label: 'Agent' },
            { v: 'total' as Dimension, label: 'Total' },
          ]}
        ...
        <Seg
          value={metric}
          options={[
            { v: 'total' as Metric, label: 'Tokens' },
            { v: 'input' as Metric, label: 'Input' },
            { v: 'output' as Metric, label: 'Output' },
          ]}
          onChange={setMetric}
        />
  ...
  return (
    <section className="insight-card">
      <header className="insight-card-header">
        ...
        <span className="insight-card-sub" title="All-source daily totals in the 31-day window outside mean ± std-dev (|z| ≥ 2); rightmost = today">|z| ≥ 2 outliers</span>
      </header>
  ...
  return (
    <>
      ...
      <header className="insight-toolbar">
        ...
        <Seg
          value={bucket}
          options={[{ v: 'day' as const, label: 'Day' }, { v: 'hour' as const, label: 'Hour' }]}
          onChange={setBucket}
        />
        <input
