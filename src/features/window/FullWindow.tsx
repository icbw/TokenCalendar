...
import { events, windowService } from '../../services'
...
export default function FullWindow() {
  ...
  const [settingsTab, setSettingsTab] = useState<'general' | 'appearance' | 'data' | 'subscriptions'>('general')
  const [widgetVisible, setWidgetVisible] = useState(false)
  // 悬浮球可见性（顶栏 Orbit 按钮；与托盘/设置页同源单一广播）
  const [orbVisible, setOrbVisible] = useState(false)
  const [maximized, setMaximized] = useState(false)

  // orb「Manage subscriptions」导航中转消费（openMainAtView 写入
  // sessionStorage,show main 唤起后此处读取一次并清除——首帧渲染前同步取,无闪切）
  useLayoutEffect(() => {
    try {
      const raw = sessionStorage.getItem('tokencalendar.main.nav')
      if (raw) {
        sessionStorage.removeItem('tokencalendar.main.nav')
        const nav = JSON.parse(raw) as { view?: string; tab?: string }
        if (nav.view === 'settings') {
          const t = nav.tab
          setSettingsTab(
            t === 'general' || t === 'appearance' || t === 'data' || t === 'subscriptions' ? t : 'general',
          )
          setView('settings')
        }
      }
