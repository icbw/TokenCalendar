// WidgetWindow：挂件窗口薄壳，仅渲染年度热力图卡片；透明由 html.mode-widget 保证
// （本窗口恒为挂件形态）。与主窗口独立运行：设计偏好（含自定义卡片色）经 localStorage
// 跨窗口同步，挂件订阅 designPrefs 即时生效；显隐状态经 Rust 广播（widget-visibility-changed）。
...
export default function WidgetWindow() {
  useShowOnLoad()
  useWidgetThemeSync()
  // 毛玻璃材质档（挂件自己的 widgetMaterial 键，设置页改键经 storage 桥广播到这里应用；
  // DWM 边缘原子组合不动，见 effects.rs 注释）。
  useMaterialSync('widget')
  // 圆角方案档（widget 值 → --widget-card-radius；与主面板同档对齐）。
  useRadiusSchemeSync('widget')

  // 挂件窗口全程透明：宿主层不得有不透明默认背景
  useLayoutEffect(() => {
